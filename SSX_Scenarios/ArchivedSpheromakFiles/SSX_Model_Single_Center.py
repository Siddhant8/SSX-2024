...
if domain_setup.getfloat('zcenter'):
    xCenter = domain_setup.getfloat('xcenter')
    yCenter = domain_setup.getfloat('ycenter')
    zCenter = domain_setup.getfloat('zcenter')
    aa_x, aa_y, aa_z = spheromak_A(domain, center = (xCenter, yCenter, zCenter))
    print("It is making it here")
else:
...
Ax['g'] = aa_x*(1 + delta*x*np.exp(-(z - zCenter)**2))
Ay['g'] = aa_y*(1 + delta*x*np.exp(-(z - zCenter)**2))
Az['g'] = aa_z*(1 + delta*x*np.exp(-(z - zCenter)**2))
...
for i in range(x.shape[0]):
    ...
    for j in range(y.shape[1]):
       ...
       for k in range(z.shape[2]):
            zVal = z[0,0,k] 
            fullGrid[i][j][k] = -np.tanh(6*(zVal - 6.0))*(1 - rho_min)/2 + np.tanh(6*(zVal - 4.0))*(1 - rho_min)/2 + rho_min
            #density in the z direction with tanh transition 
           # The initial density does not sum to 1. It was origionally wrong.
##########################################################################################################################################
...
field_writes = solver.evaluator.add_file_handler('fields_center_update_v2', max_writes = field_max_writes, sim_dt = output_cadence, mode = 'overwrite')
...
parameter_writes = solver.evaluator.add_file_handler('parameters_center_update_v2', max_writes = parameter_max_writes, sim_dt = output_cadence, mode = 'overwrite')
...
load_writes = solver.evaluator.add_file_handler('load_data_center_update_v2', max_writes = load_max_writes, sim_dt = output_cadence, mode = 'overwrite')
