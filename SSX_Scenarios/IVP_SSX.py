...
Dedalus 3 edits made by Siddhant Ranka. Direct all queries to askeldo1@swarthmore.edu or sranka1@swarthmore.edu.
...
#Problem
SSX = d3.IVP([v, A, T, lnrho, p, phi, tau_A, tau_p], time=t, namespace=locals())
...
SSX.add_equation("dt(lnrho) = 0")

# Continuity
#SSX.add_equation("dt(lnrho) = -div(v) - v@grad(lnrho)")

# Momentum
SSX.add_equation("dt(v) + grad(T) + grad(p) - nu*lap(v) = T*grad(lnrho) - v@grad(v) + cross(j,B)/(np.exp(lnrho))")
